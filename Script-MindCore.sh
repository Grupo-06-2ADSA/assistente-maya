...
executar_consulta() {
    ...
    local query_result
    query_result=$(docker exec bd-mindcore bash -c "FK_EMPRESA=\$(MYSQL_PWD=$PASSWORD mysql --batch -u root -D $DATABASE -e \"SELECT fkEmpresa FROM Funcionario WHERE email = '$email' AND senha = '$senha' LIMIT 1;\"); echo \$FK_EMPRESA")
    echo "$query_result"
}
